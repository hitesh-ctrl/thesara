...
- **Bring your own key** — when the shared daily quota is reached, users can supply their own free Groq key; the extension walks through the disclosure before asking
- **In-memory cache** — each word is fetched once per session; a bounded LRU cache (max 500 entries) prevents memory growth
...
Install from the [Chrome Web Store](https://chromewebstore.google.com) — search for **Thesara**.
...
`DictionaryService` calls `api.dictionaryapi.dev` — a free, open dictionary API. No key required. Results are cached in a `BoundedCache<string, DictionaryResult>` keyed by lowercase word, so each unique word is fetched at most once per session.
...
`VideoController` uses a depth counter rather than a boolean flag. Every hover increments the counter; every dismiss decrements it. The video resumes only when the counter reaches zero, which prevents rapid hovers from leaving the video stuck paused. The controller only resumes if the extension paused the video — a pre-existing user pause is respected.
